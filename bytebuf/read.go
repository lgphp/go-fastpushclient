...
func (b *ByteBuf) ReadBytesForSize(size int) []byte {
	readbyte := make([]byte, size)
	b.GetBytes(readbyte, b.readerIndex, b.readerIndex+size)
	b.readerIndex += size
	return readbyte
}
...
func (b *ByteBuf) ReadBytes(p []byte) (int, error) {
	n, _ := b.GetBytes(p, b.readerIndex, b.readerIndex+len(p))
	b.readerIndex += n
	return n, nil
}
...
func (b *ByteBuf) ReadBytetoDstBufOfAvailabeByte(buf *ByteBuf) error {
	return b.ReadByteToDstBuf(buf, b.readerIndex, b.writerIndex)
}

func (b *ByteBuf) ReadByteToDstBuf(buf *ByteBuf, start int, end int) error {
	if start > b.writerIndex || start < b.readerIndex {
		return ErrOutOfBound
	}
	if end <= start || end > b.writerIndex {
		return ErrOutOfBound
	}
	length := end - start
	newBytes := make([]byte, length)
	_, err := b.GetBytes(newBytes, start, end)
	buf, _ = NewByteBufWithCapatity(int32(length), newBytes)
	return err
