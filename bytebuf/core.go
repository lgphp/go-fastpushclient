...
func NewByteBufWithCapatity(capacity int32, bufs ...[]byte) (*ByteBuf, error) {
	if capacity < 1 {
		ChunkSize = int(1)
	} else {
		ChunkSize = int(capacity)
	}
	buf, e := NewByteBuf(bufs...)
	return buf, e
...
func (b *ByteBuf) SetReaderIndex(index uint) {
	b.readerIndex = int(index)
}
...
func (b *ByteBuf) RefCnt() int {
	return b.refCount
}

func (b *ByteBuf) Retain() {
	b.refCount++
}
